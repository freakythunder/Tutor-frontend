...
import styles from '@/app/Styles/IDE.module.css';
import { executeCode } from '@/app/services/codeService';
...
const IDE = forwardRef<IDERef, IDEProps>(({ height, onRun }, ref) => {
  ...
  */
  ...
  const handleRunCode = async () => {
    ...
    if (code) {
      try {
        const result = await executeCode(code); // Pass code only if it's defined
      ...
      onRun('Error: No code to execute.');
      setIsLoading(false);
    }
  };
